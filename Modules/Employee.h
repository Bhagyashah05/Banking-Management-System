...
label1:
    ...
    if(loginEmployee(connectionFD, empID, password))
    {
        ...
        struct Employee e;
        int file2 = open(EMPPATH, O_RDONLY | O_CREAT, 0644);
        lseek(file2, 0, SEEK_SET); 
        while (read(file2, &e, sizeof(e)) > 0) {
        if (e.empID == empID) {
            bzero(readBuffer, sizeof(readBuffer));

            bzero(writeBuffer, sizeof(writeBuffer));
            snprintf(writeBuffer, sizeof(writeBuffer), "\n------ Welcome %s %s------ ^", e.firstName,e.lastName);
            write(connectionFD, writeBuffer, sizeof(writeBuffer));
            read(connectionFD, readBuffer, sizeof(readBuffer));
...
void addCustomer(int connectionFD) {
    ...
    strcpy(customer.password, crypt(readBuffer, HASHKEY));
     ...
     struct Customer c;
    int isUnique = 0;
    int file2 = open(CUSPATH, O_RDONLY | O_CREAT, 0644);
    ...
    while (1) {
        ...
        while (read(file2, &c, sizeof(c)) > 0) {
            if (c.accountNumber == customer.accountNumber) {
                isUnique = 0;  
                bzero(readBuffer, sizeof(readBuffer));

                bzero(writeBuffer, sizeof(writeBuffer));
                strcpy(writeBuffer, "Account number already exists. Please enter a different number. ^");
                write(connectionFD, writeBuffer, sizeof(writeBuffer));
                read(connectionFD, readBuffer, sizeof(readBuffer));
                break; 
            }
